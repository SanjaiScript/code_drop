...
int main()
{
    ...
    int** arr = malloc ( row * sizeof(int*) );      // Row is dynamic

    for( int i = 0; i<row; i++ )
    {
        arr[i]=malloc(col * sizeof(int));           // column is also dynamic
    }
    ...
    printf("Enter the new row value\n");
    scanf("%d",&n_row);
    ...
    arr = realloc( arr , n_row * sizeof(int*) );
